...
type FoundDataManagerGenericProps = {
  entityId: string; // ID do cedente ou CNPJ do sacado
  entityType: 'cedente' | 'sacado'; // Tipo de entidade
  items: FoundDataItem[];
  onRefresh: () => void;
};
...
export default function FoundDataManagerGeneric({ 
  entityId, 
  entityType, 
  items, 
  onRefresh 
}: FoundDataManagerGenericProps) {
  ...
  const tableName = entityType === 'cedente' 
    ? 'cedentes_dados_encontrados' 
    : 'sacados_dados_encontrados';
  
  const foreignKeyColumn = entityType === 'cedente' 
    ? 'cedente_id' 
    : 'sacado_cnpj';
