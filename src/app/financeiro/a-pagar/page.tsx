...
export default function APagarPage() {
  ...
  const emAtraso = lancamentos.filter(l => l.status === 'atrasado' || (l.status === 'pendente' && new Date(l.data_vencimento || '') < new Date())).reduce((acc, l) => acc + l.valor, 0);
      ...
      <div className="container max-w-7xl space-y-6">
        <header className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-600">Gestão de despesas e valores a pagar</p>
            </div>
        ...
        <Card>
          <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900">Lançamentos a Pagar</h3>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredLancamentos.map((lancamento) => (
                  <tr key={lancamento.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                      {lancamento.data_vencimento ? new Date(lancamento.data_vencimento).toLocaleDateString('pt-BR') : 'N/A'}
                    </td>
